...
### DESCRIPTION: Code takes traffic video feed, detect vehicles and analyzes it's speed. If vehicle overspeeds(speed limit = 60 KMPH), it captures vehicles image, and tries to detect vehicle number plate and stores the same in dataframe.


### ISSUES: The video used as input has low resolution, so model was unable to detect number plates of overspeeding vehicles.
